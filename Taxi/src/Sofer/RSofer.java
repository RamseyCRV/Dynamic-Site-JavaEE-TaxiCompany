...
/**
 * Servlet implementation class RSofer
 */
@WebServlet("/RSofer")
public class RSofer extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			String reg = "insert into soferi"
					+ "(Id,Nume,Prenume,Cnp,SerieCi,NrCi,AnPermis,User,Password)"
					+ "values('" + Id + "','" + Nume + "','" + Prenume
					+ " ','" + Cnp + " ','" + SerieCi + " ','" + NrCi
					+ "','" + AnPermis + "','" + User + "','" + Password + "')";
			s.executeUpdate(reg);
			s.close();
		} catch (Exception e) {
			e.printStackTrace();
